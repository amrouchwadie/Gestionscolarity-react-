import React from 'react';

export const Coursedoc = () => (
  <div>
    <div class="container-fluid px-1 px-md-5 px-lg-1 px-xl-5 py-5 mx-auto">
    <div class="card card0 border-0">
        <div class="row d-flex">
            <div class="col-lg-6">
                <div class="card1 pb-5">
                    <div class="row">  </div>
                    <br /><br /><br /><br /><br /><div class="row px-3 justify-content-center mt-4 mb-5 border-line"> <img src="https://unblast.com/wp-content/uploads/2018/10/Back-to-School-Illustration.jpg" class="image"/> </div>
                </div>
            </div>
            <div class="col-lg-6">
                <div class="card2 card border-0 px-4 py-5">
                    <div class="row mb-4 px-3">
                        <h6 class="mb-0 mr-4 mt-2">Upload Files</h6>
                        <div class="facebook text-center mr-3">
                            <div class="fa fa-file-pdf-o"></div>
                        </div>
                        <div class="twitter text-center mr-3">
                            <div class="fa fa-file-excel-o"></div>
                        </div>
                        <div class="linkedin text-center mr-3">
                            <div class="fa fa-file-powerpoint-o"></div>
                        </div>
                    </div>
                    <div class="row px-3 mb-4">
                        <div class="line"></div> <small class="or text-center">Or</small>
                        <div class="line"></div>
                    </div>
                     <label class="mb-1">
                            <h6 class="mb-0 text-sm">Name</h6>
                        </label> <input class="mb-4" type="text" name="Name" placeholder="Enter a valid Name"/>
                  <label class="mb-1">
                            <h6 class="mb-0 text-sm">Description</h6>
                        </label> <textarea type="text" name="password" placeholder="Enter Description"></textarea>
                     <label class="mb-1">
                            <h6 class="mb-0 text-sm">Upload Files</h6>
                        </label> <input type="file" name="password" placeholder="Enter password"/>
                      <button type="submit" class="btn btn-blue text-center"> <h4 style={{color:"white"}}> Submit </h4> </button>
                     </div>
            </div>
        </div>
        <div class="bg-blue py-4">
            <div class="row px-3"> <small class="ml-4 ml-sm-5 mb-2">Upload all the files</small>
                <div class="social-contact ml-4 ml-sm-auto"> <span class="	fa fa-file-zip-o mr-4 text-sm"></span> <span class="	fa fa-file-o mr-4 text-sm"></span> <span class="	fa fa-file-movie-o mr-4 text-sm"></span> <span class="	fa fa-file-code-o mr-4 mr-sm-5 text-sm"></span> </div>
            </div>
        </div>
    </div>
</div>
  </div>
)
